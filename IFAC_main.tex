...
\end{figure}
In the figure, one experiment (the one corresponding to the line segment shape) has been omitted 
...
Still 
the quadrotor is not exactly in the desired position, denoted with the red star.
...
the formation errors for the first experiment plotted in~\ref{fig:exp_noObstacles_1} 
(only this one, for the sake of brevity).
\begin{figure}
    \centering
    \begin{subfigure}[b]{0.49\columnwidth}
        \centering
...
The same behaviour has been observed in all the tests.
% , showing that there 
