...
\begin{equation}
    \footnotesize
    J(k) = \boldsymbol{u}(k)^TQ_u\boldsymbol{u}(k)+\boldsymbol{e}(k+1)^TQ_x\boldsymbol{e}(k+1)+\boldsymbol{e}(k)^TQ_x\boldsymbol{e}(k)   
\end{equation}
...
\footnotesize
    ...
    \end{bmatrix}^T \begin{bmatrix}
        Q_x + A_d^T Q_x A_d & A_d^T Q_x B_d \\ 
        B_d^T Q_x A_d & Q_u + B_d^T Q_x B_d
...
with
\begin{eqnarray}
    Q_{xx} &=& Q_x + A_d^T Q_x A_d \\
    Q_{xu} &=& A_d^T Q_x B_d,\ Q_{ux} = Q^T_{xu} \\
    Q_{uu} &=& Q_u + B_d^T Q_x B_d
\end{eqnarray}
Note that the problem~\eqref{eq:minmax_v1} has the same form as 
the one considered by Gattami et al.~\cite{gattami_robust_2012}. 
\cite{gattami_robust_2012} prove that the linear policy is optimal, i.e.
\begin{equation}
...
\begin{equation}
    ...
    \begin{split}
        ...
        &\quad \left(
            \begin{matrix}
                Q_{xx} - \gamma I + Q_{xu} K C + C^T K^T Q_{xu}^T & C^T K^T \\
                KC & -Q_{uu}^{-1}
            \end{matrix}
        \right) \leq 0
    \end{split}
...
is clean and collisions may occur between $R_1$ and $R_2$.
On one hand, since $R_1$ has the highest priority, 
there is no need to correct the control action 
$\boldsymbol{u}^\star_{1}(k)$.
On the other hand, the robot R2 has to correct 
its way to avoid the other ground vehicle.
A repulsive force can be used to attain this,
with a direction opposite to the vector going from $R_2$ to $R_1$
...
by the obstacle.
The final control inputs for $R_2$ are:
\begin{equation}
\begin{aligned}
