...
\usepackage{hyperref} % Load hyperref here

\makeatletter
\let\@ssect\old@ssect % 3. Restore the IFAC's original command
\makeatother
...
\title{LMI-based Formation Control with Collision Avoidance for Air-Ground Fleet} 
...
The formation problem is defined as a min-max problem whose optimal control strategy is linear 
in the local measurements of each agent, and the matrix gains are obtained by solving a Linear Matrix Inequality.
...
\end{keyword}
% https://ifac.micehub-gov.com/fairContents.do?FAIRMENU_IDX=21837&hl=ENG#guide-keywords

\end{frontmatter}
%===============================================================================

\section{Introduction}
Multi-agent systems (MASs) are nowadays used in a variety of applications as
they can perform complex tasks, which would be otherwise impossible for a single robot.
This is evident when considering an heterogeneous fleet where the strengths
...
to another while being supervised from above by a drone with a wide field of view.
A deeper analysis of the subject to understand the potential tasks and challenges of MASs,
the reader is referred to the work of~\cite{Maldonado2024}.
...
For example, ~\cite{LEE200985} already proposed
a decentralized control algorithm for a team of two-wheeled robots 
to achieve a geometric pattern.
However, formation control remains an active research topic.
...
In the recent years, reinforcement learning has been used in many areas including 
formation control. 
...
of a team of quadrotors.

This is the context of our work, which proposes a novel formation control scheme
for a group consisting of a quadcopter and two unicycles, as shown in Fig.~\ref{fig:summary_image}.
\begin{figure}
    \centering
    % trim = left bottom right top (in points or any unit)
    % clip = true ensures the parts outside the box are hidden
    % \includegraphics[width=\columnwidth, trim={0 50 0 50}, clip]{images/imageIntro2.png}
    \includegraphics[width=\columnwidth, trim={0 75 0 75}, clip]{images/imageIntro5.png}
    \caption{Heterogeneous multi-agent setup for the experimental validation of the proposed formation control, including collision and obstacle avoidance.}
...
LMIs are a powerful mathematical tool used in many applications, among which is also 
formation control.
In the work of~\cite{Trejo2023}, a LMI is solved to design the controller 
...
the solvability of a LMI.
~\cite{Semsar2009} present a LMI formulation of the LQR problem 
...
where a control architecture combining Feedback Linearization (FL), 
...
simulations and a comprehensive set of experiments in an indoor arena, including both static
...
Still, if a point located at distance $R > 0$ from 
$(x_i, y_i)$, which will be denoted as $({x}_{R_i}, {y}_{R_i})$, is 
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Moving on to the drone, since the formation problem is restricted to the $(X,Y)$-plane, 
some simplifying hypothesis have been made.
First, let us assume that there is an inner attitude controller ensuring attitude stability.
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Let us analyze the equations with the help of Fig.~\ref{fig:drone_roll_pitch_frames}.
...
the thrust produced by the motors $T$ along the $k^B$ direction and 
...
\begin{figure}
    ...
    \begin{subfigure}{0.31\columnwidth}
        % trim = left bottom right top (in points or any unit)
        \hspace{0.5cm}
        \includegraphics[trim={0.8cm 2cm 0cm 2cm}, clip, width=2.7cm]{images/drone_fromAbove2.png}
    ...
    \begin{subfigure}{0.31\columnwidth}
        ...
        % \vspace{-1.3cm}
        \includegraphics[trim={0cm 0cm 0.5cm 0cm}, clip, width=2.7cm]{images/drone_roll3.png}
        % \vspace{0.7cm}
    ...
    \begin{subfigure}{0.31\columnwidth}
        ...
        % \vspace{0.9cm}
        % \vspace{-0.01cm}
        \includegraphics[trim={0.8cm 0.1cm 0cm 0.5cm}, clip, width=2.7cm]{images/drone_pitch2.png}
        % \vspace{0.1cm}
        % \vspace{0.9cm}
...
\end{figure}
Let us suppose that the $Z$-controller 
\begin{equation}
...
\begin{equation}
    \boldsymbol{e}(k+1) = A_d \boldsymbol{e}(k) + B_d \left[\boldsymbol{u}(k) + \boldsymbol{d}(k)\right]
    \label{eq:airground-fleet-z}
\end{equation}
...
\subsection{Formation Maintenance: a LMI Approach}
...
\begin{subequations}
    ...
    \boldsymbol{z}_A = \begin{bmatrix}
        e_{A,x}  \\ e_{A,y} \\ \dot{e}_{A,x} \\ \dot{e}_{A,y}
...
the local measurement $\boldsymbol{z}_{i}(k)$ by $K^\star_i$ to obtain $\boldsymbol{u}^\star_{i}(k)$.
% \textcolor{red}{spiega che LMI si deve risolvere una sola volta}
...
It is therefore crucial to propose a solution to avoid crashes 
...
On the other hand, the robot $G_2$ has to adjust 
...
The tests have been carried out on a laptop with OS Ubuntu 20.04.6 LTS, 
...
%         \includegraphics[width=\textwidth]{images/robot_dSafe.pdf}
%     \end{subfigure}
%     % \hspace{-0.6cm}
%     \begin{subfigure}{0.35\columnwidth}
%         \includegraphics[width=\textwidth]{images/dSafe.pdf}
%     \end{subfigure}
    % \vspace{-0.35cm}
    \includegraphics[width=0.75\columnwidth]{images/collisions3.png}
...
\begin{equation}
    ...
    \end{aligned} \right.
\end{equation}
with $\varepsilon$ being a negligible error. For the simulations, it has been set to
...
For each unicycle, the 2D-trajectories of the center of rotation 
are plotted along with the point $(x_{R_i}, y_{R_i})$ 
with a solid and a dotted line, respectively.
The desired inter distances drone-$G_1$ and $G_1$-$G_2$ are plotted 
in dotted black, 
while the actual inter distances at the end of the simulation 
...
The key performance index for the first scenario is $\bar{t} = \SI{59.38}{\second}$ and for 
the second case $\bar{t} = \SI{59.38}{\second}$.
...
\begin{figure}
    ...
    \end{subfigure}
    \vspace{-0.5cm}
    \caption{Trajectories of the three agents without APFs in ideal simulations.}
...
Fig.~\ref{fig:sim_withAPF_noNoise_2} and Fig.~\ref{fig:sim_withAPF_noNoise_3}, respectively.
Without APFs, the unicycles get too close and they must stop.
Hence, even if the drone reaches the desired position $(x^d_A, y^d_A)$, 
the overall formation is not attained by the fleet.
On the contrary, thanks to the artificial repulsive forces the robot $G_2$ 
...
This is the reason why the solid and dotted lines do not exactly overlap:
the solid shape appears indeed to be a translated version of the dotted one.
However, collisions between $G_1$ and $G_2$ are avoided and the formation is reached after
% $\bar{t} = \SI{54.22}{\second}$, 
$\bar{t} = \SI{112.14}{\second}$ and 
...
the next step was to validate the formation control scheme 
through experiments.
...
The control actions were computed into separate ROS nodes running on 
a ground station (i.e., the same laptop used for the simulations) 
then sending the commands to the separate robots.
For controlling the quadcopter, the implementation of the inner controller 
has been done using the FL-AIR framework developed by~\cite{SanahujaFlair}.
...
Making an analogy with a PD controller, we would indeed like the gain $K^\star_A$
...
the $X$-dynamics and one for the $Y$-dynamics.
Moreover, when tuning experimentally a PD attitude controller 
for the quadcopter, good  performance were obtained 
...
\begin{aligned}
    ...
    % \quad & \boldsymbol{u}_{2}(k) = \boldsymbol{\mu}_2(\boldsymbol{z}_2(k))\\
\end{aligned}
\label{eq:minmax_v2}
\end{equation}
...
The first experiments have been conducted with the ground free of obstacles.
...
\end{figure}
In the figure, an experiment (the one corresponding to the line segment shape) 
has been omitted 
due to limited space.
The geometric shape is achieved by the fleet.
...
$\bar{t} = \SI{8.92}{\second}$ and
% $\bar{t} = \SI{10.04}{\second}$,
$\bar{t} = \SI{10.34}{\second}$ in the two scenarios respectively.
However, the quadrotor is not exactly in the desired position, denoted by the red star.
% Indeed, let us analyze the formation errors shown in 
% Fig.~\ref{fig:exp_noObstacles_formationErrors}.
Indeed, let us analyze Fig.~\ref{fig:exp_noObstacles_formationErrors}, which shows
the formation errors for the first experiment shown in~\ref{fig:exp_noObstacles_1} 
(considering only this case for the sake of brevity).
\begin{figure}
    \centering
    \begin{subfigure}[b]{0.49\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{images/experiment/nominal/error_x.eps}
        \caption{Triangle Top}
        \label{fig:exp_noObstacles_error_x}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{images/experiment/nominal/error_y.eps}
        \caption{Triangle Right}
        \label{fig:exp_noObstacles_error_y}
      \end{subfigure}
   \vspace{-0.5cm}
   \caption{Formation errors of the team corresponding to the triangle-top shape 
   in the $X$ and $Y$ directions, shown on the left and right, respectively.}
   \label{fig:exp_noObstacles_formationErrors}
\end{figure}
The ground robots'errors, shown in blue and green, converge to zero, while the drone maintains
a non-zero error that is still smaller than $\varepsilon$.
The same behavior has been observed in all the tests.
% , showing that there 
...
respectively.
For these reasons, the final geometry of the team (solid line)
in Fig.~\ref{fig:exp_noObstacles}
maintains the correct shape and orientation but translated from the desired configuration 
(dotted black line).
...
A video collecting the experiments can be found at:
\textcolor{blue}{\href{https://youtu.be/wefGtmPAPiA}{https://youtu.be/wefGtmPAPiA}}.
This time, the test with the line segment was also included. 
In this scenario, the geometric shape is achieved at $\bar{t} = \SI{10.04}{\second}$.
At the end of all experiments, even the ground vehicles appear to oscillate. 
This can be explained by the small oscillations of the drone around the desired position, 
which are then mirrored by the unicycles as they maintain the geometric shape.
To prove this supposition, a trial was performed using only the ground robots, 
...
Let us compare it with Fig.\ref{fig:exp_noObstacles_1}.
...
and $\bar{t}_2 = \SI{8.92}{\second}$, respectively, with:
\begin{equation}
    \forall t \geq \bar{t}_1 \ : \ \left\lvert e_{\Delta_{A1,x}}(t) \right\rvert < \varepsilon , \left\lvert e_{\Delta_{A1,y}}(t)\right\rvert < \varepsilon 
\end{equation}
\begin{equation}
    \forall t \geq \bar{t}_2 \ : \ \left\lvert e_{\Delta_{12,x}}(t) \right\rvert < \varepsilon , \left\lvert e_{\Delta_{12,y}}(t) \right\rvert < \varepsilon
\end{equation}
...
To confirm this assumption, another experiment was conducted in which the ground 
robots'starting positions were swapped, so that the robots began on the same side 
as their respective desired final positions.
Fig.~\ref{fig:exp_staticObstacles_left} shows the results.
The vehicle $G_2$ this time avoids the obstacles moving on 
the left-hand side of the cones.
This is reflected in the key performance indexes:
$\bar{t} = \SI{9.66}{\second}$,
$\bar{t}_1 = \SI{8.20}{\second}$, and $\bar{t}_2 = \SI{9.66}{\second}$.
A video with the two experiments can be seen at:
\textcolor{blue}{\href{https://youtu.be/_DwPV7lcr4U}
{https://youtu.be/\_DwPV7lcr4U}}.
...
\end{figure}
At the beginning of the experiment, the dynamic obstacle is far from the agents,
allowing them to reach 
the formation, as in the scenario free of obstacles 
in Fig.~\ref{fig:exp_noObstacles_3}.
Subsequently, however, the jammer moves closer to the Jetbots.
When the remote-controlled robot gets close to $G_2$, this last moves away, 
...
\textcolor{blue}{\href{https://youtu.be/vXEV-TBfdAY}{https://youtu.be/vXEV-TBfdAY}}.

\section{Conclusion}
\label{sec:conclusion}
This paper continues the work by~\cite{Morando2025SOSE}
and addresses the formation of a heterogeneous fleet composed 
of two unicycles and a quadcopter.
The proposed solution combines feedback linearization,
a LMI-based controller, and APFs.
The robust formation achievement problem is defined as 
a linear-quadratic dynamic game, where the optimal control 
strategy is obtained by solving only one time offline a LMI.
To avoid collisions among the ground robots and with any
obstacles, a hierarchy is defined between the two-wheeled agents, 
and APFs are employed.
After some simulations, the proposed scheme has  been extensively validated 
through experiments in an indoor arena.
The test set included scenarios with static and dynamic obstacles.
Results show that in all the cases, the team is able to reach 
the formation without the crashes of the ground robots.
Still there is room for improvement, which opens up to new perspectives.
First, to reduce the position tracking error of the drone, one approach  
is to introduce two additional dynamics: an integral 
action on the $X$ and an integral action on the $Y$.
In this way, an analogy can be drawn with a PID controller, similar to the one
used with the PD controller.
Finally, in this work it was assumed that the Z-controller 
compensates the gravity, which results in $(X,Y)$-dynamics 
independent of the drone's mass.
For future work, it would be interesting to include 
the quadrotor's altitude in the formation dynamics 
to address changes in the mass, potentially 
by modelling the overall system as a Linear Parameter-Varying one.
